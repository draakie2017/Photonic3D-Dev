...
git clone https://github.com/draakie2017/Photonic3D-Dev.git photonic-repo
...
if [ "$OCTOPI_PHOTOCENTRIC_HARDWARE" == "4kscreen" ]
	then
		...
		wget https://raw.githubusercontent.com/draakie2017/Photonic3D-Dev/master/host/printers/Photocentric%20Pro.json -O printerprofile.json
...
if [ "$OCTOPI_PHOTOCENTRIC_HARDWARE" == "LCHR" ]
	then
		...
		wget https://raw.githubusercontent.com/draakie2017/Photonic3D-Dev/master/host/printers/photocentric%20hr.json -O printerprofile.json
		curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d @printerprofile.json 'http://localhost:$portno/services/printers/save'
...
if [ "$OCTOPI_PHOTOCENTRIC_HARDWARE" == "standalone" ]
	then
		...
		wget https://raw.githubusercontent.com/draakie2017/Photonic3D-Dev/master/host/printers/photocentric%2010.json -O printerprofile.json
		curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d @printerprofile.json "http://localhost:$portno/services/printers/save"
